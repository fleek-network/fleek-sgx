...
fn main() -> anyhow::Result<()> {
    ...
    // generate epehmeral key
    // get pk of that
    // get quote & collat for [pk; 64]
    // build tls client which sends our tls extension
    // make requests to nodes
    // get SecretKey
    // build tls server (uses same ephemeral key)
    // reuse sending quote/collat from before
    //
    // build
    //
    ...
    hasher.update(pub_key.to_pkcs1_der()?.as_bytes());
